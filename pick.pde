...
public pt pick(float mX, float mY, float mZ) { 
  ...
  void showFloor() 
    {
    fill(grey); pushMatrix(); translate(0,0,-1.5); box(800,800,1); popMatrix(); // draws floor as thin plate
    }
    
  void doPick()
    {
    Ofp.setTo(Of);
    // places Of on the floor under the mouse  
   ...
   if(mousePressed&&!keyPressed) // when mouse is pressed (but no key), show red ball at surface point under the mouse (and its shadow)
    ...
    PathOne.setIdOfVertexWithClosestScreenProjectionTo(Mouse()); // ID of vertex of P with closest screen projection to mouse (us in keyPressed 'x'...
...
//    pushMatrix(); fill(grey,100); scale(1,1,0.01); P.drawBalls(4); Q.drawBalls(4); PtQ.drawBalls(4); popMatrix(); // show floor shadow of green balls
      fill(green); PathOne.drawBalls(4); // draw green balls
 //   fill(red); Q.drawBalls(4); // draw green balls
//    fill(blue); PtQ.drawBalls(4); // draw green balls
      fill(red,100); PathOne.showPicked(6); // shows currently picked vertex in transparent red (last key action 'x', 'z'
       }
    }
