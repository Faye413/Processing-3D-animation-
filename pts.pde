...
class pts // class for manipulaitng and displaying pointclouds or polyloops in 3D 
  ...
  pts empty() {nv=0; pv=0; return this;}                                 // resets P so that we can begin adding points
  ...
  pts copyFrom(pts Q) {empty(); nv=Q.nv; for (int v=0; v<nv; v++) {
  G[v]=P(Q.G[v]);DirectionControl[v]=Q.DirectionControl[v];} return this;} // set THIS as a clone of Q
  ...
  //added to control path 
  int nextPOINT(int i) {return (i+1)%nv;}
  int previousPOINT(int i) {return (i-1+nv)%nv;}
  int[] DirectionControl = new int[maxnv];        
  void PathControl(int input) {
    DirectionControl[pp] = input;
  }
  void drawCurve() {
    if (loop){
      beginShape(); 
  ...
  void lineFunc() {
    for (int v=0;v<nv;v++) {
      show(P(G[v].x,G[v].y,3),v+"");
      color c = (DirectionControl[v]==0)?green:((DirectionControl[v]==1)?orange:red); pen(c,5);
      showShadow(G[v],12); //draw dot
    }
  }
  ...
  pts insertPt(pt P) { // inserts new vertex after vertex with ID iv
    for(int v=nv-1; v>iv; v--) {G[v+1].setTo(G[v]); DirectionControl[v+1]=DirectionControl[v];}
     iv++; 
     G[iv].setTo(P);
     DirectionControl[iv] = 0;
  ...
  pts drawBalls(float r) {for (int v=0; v<nv; v++){color c = (DirectionControl[v]==0)?blue:(DirectionControl[v]==1)?green:orange; fill(c);show(G[v],r);} return this;}
  ...
  pts setPicked() {pv=pp; return this;}
  ...
  void savePts(String fn) 
    ...
    int s=0;
    inppts[s++]=str(nv);
    //modified for direction control 
    for (int i=0; i<nv; i++) {inppts[s++]=str(G[i].x)+","+str(G[i].y)+","+str(G[i].z)+","+str(DirectionControl[i]);}
  ...
  void loadPts(String fn) 
    ...
    for(int k=0; k<nv; k++) {int i=k+s; float [] xy = float(split(ss[i],",")); G[k].setTo(xy[0],xy[1],xy[2]); /*D[k] = int(xy[3]);*/}
    pv=0;
    }; 
